...
<template>
    <v-btn variant="outlined" color="primary" @click="openModal = true" class="mb-2 mt-2" prepend-icon="mdi-plus">Agregar</v-btn>
                ...
                <v-form>
                    <template v-for="field in props.fields" :key="field.value">
                        ...
                        <template v-if="field.type === 'select'">
                            <v-select  variant="solo-filled" :items="field.options" v-model="data[field.value]" item-title="title" 
  item-value="value" />
                        </template>
                        <template v-else>
